...
class MakeRepositoryCommand extends Command
{
    protected $signature = 'make:repository
                            {name : The model name for the repository}';

    protected $description = 'Create a repository class for an Eloquent model';
    ...
    public function handle(): int
    {
        try {
            ...
            $target = $this->path("app/Repositories/{$class}Repository.php");

            if ($this->files->exists($target)) {
                throw new RuntimeException("Repository [{$target}] already exists.");
            }
            ...
            if ($this->files->put($target, $contents) === false) {
                throw new RuntimeException("Unable to write repository [{$target}].");
            }
        ...
        $this->components->info("Repository [{$class}Repository] created successfully.");
    ...
    private function normalizeName(string $name): string
    {
        ...
        if ($name === '' || ! preg_match('/^[A-Za-z][A-Za-z0-9 _-]*$/', $name)) {
            throw new RuntimeException('The repository name must start with a letter and contain only letters, numbers, spaces, dashes, or underscores.');
        }
        ...
        if (
            ...
            throw new RuntimeException("The repository name [{$name}] cannot be normalized to a valid PHP class name.");
        }
    ...
    {
        return dirname(__DIR__, 3).'/stubs/module/repository.stub';
    }
}
